...
<div style="background-image:url('<?= $C->SITE_URL.'themes/'.$C->THEME ?>/imgs/post2_backgr.gif'); background-repeat:repeat-x;">
<div style="margin-top:7px; margin-right:7px; margin-left:7px; text-align:justify;"><?= htmlspecialchars($D->usr->about_me) ?><br><br></div></div>
					<?php } ?>
					...
					<table cellpadding="0" cellspacing="3">
						...
						<?php } ?>
<?php if( !empty($D->usr->num_posts) ) { ?><tr>
<td class="contactparam"><img src="<?= $C->SITE_URL.'themes/'.$C->THEME ?>/imgs/cicons_username.gif" alt="" title="<?= $this->lang('usrlist_numposts') ?>" /></td>
<td class="contactvalue"><?= htmlspecialchars($D->usr->num_posts) ?> <?= $this->lang('usrlist_numposts') ?></td></tr><?php } ?>
