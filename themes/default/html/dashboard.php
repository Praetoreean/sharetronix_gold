...
?>
    ...
    <div id="home_content">
        <div class="ttl" style="margin-bottom:8px;">
            <div class="ttl2"><h3><?= $this->lang('ttlads') ?></h3></div>
        </div>
        ...
        <?php if ($D->tab == 'group') { ?>
            <div class="ttl" style="margin-bottom:8px;">
                <div class="ttl2">
                    <h3>
                        <a href="<?= $C->SITE_URL ?><?= $D->onlygroup->groupname ?>" title="<?= htmlspecialchars($D->onlygroup->title) ?>"><?= htmlspecialchars(str_cut($D->onlygroup->title, 50)) ?></a>
                    </h3>
        ...
        <?php } else { ?>
            ...
            <div id="pmfilter" style="display:<?= $D->privtab == 'usr' && !$D->privusr ? 'block' : 'none' ?>;">
                <form name="privform" method="POST" action="javascript:;" onsubmit="privmsg_usrfilter_setusr(this.privusr_inp.value,true); return false;">
                    <?= $this->lang('dbrd_privtab_usrtb_txt') ?>:
                    <input type="text" name="privusr_inp" value="" rel="autocomplete" autocompletecallback="privmsg_usrfilter_setusr(word);" style="width:178px;"/>
                </form>
            </div>
            <div id="pmfilterok" style="display:<?= $D->privtab == 'usr' && $D->privusr ? 'block' : 'none' ?>;">
                ...
                <a href="javascript:;" onclick="privmsg_usrfilter_reset();" onfocus="this.blur();">
                    <small><?= $this->lang('dbrd_privtab_usrtb_txt_x') ?></small>
                </a>
            </div>
            <script type="text/javascript">
                var tmpfnc = function () {
                    try {
                        document.privform.privusr_inp.focus();
                    } catch (e) {
                    }
                };
        ...
        <?php if( $D->num_results >= $C->PAGING_NUM_POSTS ) { ?>
            ...
            <div id="loadmore">
                <a href="javascript:;" id="loadmorelink" onclick="load_more_results('posts_html', <?= $C->PAGING_NUM_POSTS ?>);">ادامه ارسالها</a>
                <img id="loadmore-img" src="<?= $C->SITE_URL.'themes/'.$C->THEME ?>/imgs/loading_posts.gif" />
                <a href="javascript:;" id="loadmoretotop" onClick="scroll_to_top();">بالا</a>
            </div>
    ...
    <div id="home_right">
        ...
        <?php if ($this->user->is_logged && $this->user->info->is_network_admin == 1) { ?>
            ...
            <a href="javascript:NewWindow ('up.php','Uploader','320','270','center','front');">
                <img src="<?= $C->SITE_URL . 'themes/' . $C->THEME ?>/imgs/uploader.jpg" style="border-width: 0"></a>
            </p><?php } ?>
        ...
        <p class="style1" align="center"><a href="rss/all:posts">
                <img src="<?= $C->SITE_URL . 'themes/' . $C->THEME ?>/imgs/rss-feed.png" style="border-width: 0"></a>
        </p>
        <?php if (count($D->last_online) > 0) { ?>
            <div class="ttl" style="margin-top:0px; margin-bottom:8px;">
                <div class="ttl2"><h3><?= $this->lang('dbrd_right_lastonline') ?></h3></div>
            </div>
        ...
        <?php } ?>
        <div class="ttl" style="margin-top:0px; margin-bottom:8px;">
            <div class="ttl2"><h3><?= $this->lang('dbrd_topuser_title') ?></h3></div>
        </div>
        <div class="slimusergroup" style="margin-right:1px; margin-left:3px; margin-bottom:5px;">
        ...
        <?php { ?>
            <div class="ttl" style="margin-top:0px; margin-bottom:8px;">
                <div class="ttl2"><h3><?= $this->lang('active_groups_block') ?></h3></div>
            </div>
        ...
        <?php if (count($D->saved_searches) > 0) { ?>
            <div class="ttl" style="margin-top:0px; margin-bottom:8px;">
                <div class="ttl2"><h3><?= $this->lang('dbrd_right_savedsearches') ?></h3></div>
            </div>
        ...
        <?php if (count($D->post_tags) > 0) { ?>
            <div class="ttl" style="margin-top:0px; margin-bottom:8px;">
                <div class="ttl2"><h3><?= $this->lang('dbrd_right_posttags') ?></h3></div>
            </div>
            <div class="taglist" style="margin-bottom:5px;">
                <?php foreach ($D->post_tags as $tmp) { ?>
                    <a href="<?= $C->SITE_URL ?>search/posttag:%23<?= $tmp ?>" title="#<?= htmlspecialchars($tmp) ?>">
                        <small>#</small><?= htmlspecialchars(str_cut($tmp, 25)) ?></a>
                <?php } ?>
            </div>
        <?php } ?>
    </div>
