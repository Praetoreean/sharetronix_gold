...
function post_like(postid)
{
	...
	req.onreadystatechange	= function() {
		...
		//d.getElementById("postlink_like_"+postid).style.display	= "none";
	}
...
function post_dislike(postid)
{
	var req = ajax_init(false);
	if( ! req ) { return; }
	var thislink	= d.getElementById("postlink_dislike_"+postid);
	req.onreadystatechange	= function() {
		if( req.readyState != 4  ) { return; }
		if( req.responseText != "OK" ) { return; }
		thislink.style.cursor	= "pointer";
...
function postuserbox_close_ev(post_id) {
	postuserbox_tmout	= setTimeout( function() { postuserbox_close(post_id) }, 100 );
