<?php
	...
	$tmp	= $db2->fetch('SELECT COUNT(id) AS u, SUM(is_network_admin=1) AS a FROM users WHERE active=1');
	$D->tabnums	= array();
	$D->tabnums['all']		= $tmp->u;
	$D->tabnums['admins']		= $tmp->a;
	if( $this->user->is_logged ) {
		$tmp	= $this->network->get_user_follows($this->user->id);
		$D->tabnums['ifollow']		= count($tmp->follow_users);
		$D->tabnums['followers']	= count($tmp->followers);
	}
	...
	elseif( $tab == 'ifollow' ) {
		...
		$from	= ($D->pg - 1) * $C->PAGING_NUM_USERS;
		$usrs	= array();
		if( $sdf = $this->network->get_user_follows($this->user->id) ) {
			$usrs	= $sdf->follow_users;
		}
		$tmp	= array_keys(array_slice($usrs, $from, $C->PAGING_NUM_USERS, TRUE));
	}
