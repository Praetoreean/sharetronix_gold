		...
		if( empty($D->email) || empty($D->password) ) {
			...
			if( ! $res ) {
				...
				if( $this->network->id ) {
					...
					elseif( isset($C->LDAP_ON) && $C->LDAP_ON ) {
						require_once( $C->INCPATH.'helpers/func_signup.php' );
						$res	= user_try_login_in_ldap($D->email, $D->password, $D->rememberme);
						if( $res === TRUE ) {
							$this->redirect($C->SITE_URL.'dashboard');
							exit;
						}
						elseif( $res ) {
							$tmpkey	= md5(time().rand());
							$_SESSION['LDAP_'.$tmpkey]	= $res;
							$this->redirect($C->SITE_URL.'signup/ldapinfo:'.$tmpkey);
							exit;
						}
					}
				}
