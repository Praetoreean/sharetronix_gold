<?php
	...
	if($filter == 'all') {
		...
		$q1	= 'SELECT COUNT(p.id) FROM posts p, posts_attachments a WHERE p.id=a.post_id AND p.user_id<>0 AND p.api_id<>2 AND p.api_id<>6 '.$not_in_groups.' AND a.type="'.$at_tmp[$filter].'" ';
		$q2	= 'SELECT p.*, "public" AS `type` FROM posts p, posts_attachments a WHERE p.id=a.post_id AND p.user_id<>0 AND p.api_id<>2 AND p.api_id<>6 '.$not_in_groups.' AND a.type="'.$at_tmp[$filter].'" ORDER BY p.id DESC ';
	}
	...
	$r	= $db2->query('SELECT * FROM `groups` ORDER BY `groups`.`num_posts` DESC LIMIT 0 , 18');
	while($o = $db2->fetch_object($r)) {
