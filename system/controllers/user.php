<?php
	...
	if($D->tab == 'updates') {
		...
		$D->posts_html	= '';
		$D->filter	= 'all';
		$tmp	= array('all', 'posts', 'tweets', 'rss');
		...
		$D->show_in_updates = $reshared;
		
		$reshared	= count($reshared)==0 ? '' : ( count($reshared)==1 ? ('OR p.id="'.reset($reshared).'"') : ('OR p.id IN('.implode(', ',$reshared).')') );
		if($D->filter == 'all') {
			$q1	= 'SELECT COUNT(p.id) FROM posts p WHERE ((p.user_id="'.$u->id.'" '.$not_in_groups.') '.$reshared.')';
			$q2	= 'SELECT p.*, "public" AS `type` FROM posts p WHERE ((p.user_id="'.$u->id.'" '.$not_in_groups.') '.$reshared.') ORDER BY p.id DESC ';
		}elseif($D->filter == 'posts')
		{
			$q1	= 'SELECT COUNT(p.id) FROM posts p WHERE ((p.user_id="'.$u->id.'" '.$not_in_groups.') '.$reshared.') AND p.api_id=0 ';
			$q2	= 'SELECT p.*, "public" AS `type` FROM posts p WHERE ((p.user_id="'.$u->id.'" '.$not_in_groups.') '.$reshared.') AND p.api_id=0 ORDER BY p.id DESC ';
		}elseif($D->filter == 'tweets')
		{
			$q1	= 'SELECT COUNT(p.id) FROM posts p WHERE ((p.user_id="'.$u->id.'" '.$not_in_groups.') '.$reshared.') AND p.api_id=6'; 
			$q2	= 'SELECT p.*, "public" AS `type` FROM posts p WHERE ((p.user_id="'.$u->id.'" '.$not_in_groups.') '.$reshared.') AND p.api_id=6 ORDER BY p.id DESC ';
		}elseif($D->filter == 'rss')
		{
			$q1	= 'SELECT COUNT(p.id) FROM posts p WHERE ((p.user_id="'.$u->id.'" '.$not_in_groups.') '.$reshared.') AND p.api_id=2';
			$q2	= 'SELECT p.*, "public" AS `type` FROM posts p WHERE ((p.user_id="'.$u->id.'" '.$not_in_groups.') '.$reshared.')  AND p.api_id=2 ORDER BY p.id DESC ';
		}
	...
	elseif($D->tab=='groups' && $D->num_groups>0) {
		...
		$tmp	= array();
		$db2->query('SELECT id FROM groups WHERE id IN('.implode(',',$groups).') ORDER BY '.$sql_orderby[$D->orderby].' LIMIT '.$from.', '.$C->PAGING_NUM_GROUPS);
