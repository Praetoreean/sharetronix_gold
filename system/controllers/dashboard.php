<?php
	...
	$D->show_twitter_tab	= $db2->fetch_field('SELECT post_id FROM post_userbox_tweets WHERE user_id="'.$this->user->id.'" LIMIT 1') ? TRUE : FALSE;
	if($D->show_twitter_tab) {
		$tabs[]	= 'tweets';
	}
	...
	switch($tab)
	{
		...
		case 'tweets':
			if($filter == 'all') {
				$q1	= 'SELECT COUNT(post_id) FROM post_userbox_tweets WHERE user_id="'.$this->user->id.'" ';
		...
		case 'everybody':
			if($filter == 'all') {
				...
				$q2	= 'SELECT p.*, "public" AS `type` FROM posts p WHERE p.user_id<>0 AND p.api_id<>2 AND p.api_id<>6 '.$not_in_groups.' ORDER BY p.id DESC ';
			}
			else {
	...
	$D->tabs_state	= $this->network->get_dashboard_tabstate($this->user->id, array('all','@me','private','commented','feeds', 'tweets'));
	...
	$r	= $db2->query('SELECT * FROM `users` WHERE id = "'.$this->user->id.'" LIMIT 0 , 12');
	while($o = $db2->fetch_object($r)) {
		
		$D->my_name[]	= $this->network->get_user_by_id($o->id);
	}
	$D->my_name	= array_slice($D->my_name, 0, $num);

	$D->topten	= array();
	
	$r	= $db2->query('SELECT * FROM `users` ORDER BY `users`.`num_posts` DESC LIMIT 0 , 18');
	while($o = $db2->fetch_object($r)) {
		
	$D->topten[]	= $this->network->get_user_by_id($o->id);
	}
	$D->topten	= array_slice($D->topten, 0, $num);

	$D->grplar	= array();
	
	$r	= $db2->query('SELECT * FROM `groups` ORDER BY `groups`.`num_posts` DESC LIMIT 0 , 12');
	while($o = $db2->fetch_object($r)) {
		
	$D->grplar[]	= $this->network->get_group_by_id($o->id);
	}
