<?php
	...
	if( isset($_POST['sbm']) ) {
		...
		if( !$D->error && $f->is_read ) {
			...
			if(preg_match("/^http(s)*:\/\/(www.)*twitter.com\//iu", $D->newfeed_url)) $D->twitter_feed = TRUE;
			...
			$q .= ' AND feed_url="'.$this->db2->e($D->newfeed_url).'" AND feed_userpwd="'.$usrpwd.'"';
			$q .= ' AND filter_keywords="'.$keywords.'" LIMIT 1';
