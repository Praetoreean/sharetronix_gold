	...
	class post
	{
		...
		public function __construct($type, $load_id=FALSE, $load_obj=FALSE)
		{
			...
			$this->post_resharesnum	= 0;
                        $this->post_likesnum	= 0;
                        $this->post_dislikesnum	= 0;
			if( $obj->mentioned > 0 ) {
			...
			if( $type=='public' && $obj->reshares > 0 ) {
				$r	= $this->db2->query('SELECT user_id FROM posts_reshares WHERE post_id="'.$obj->id.'" ORDER BY id DESC LIMIT '.$obj->reshares, FALSE);
				while($o = $this->db2->fetch_object($r)) {
		...
		public function reshare_post()
		{
			...
			$users	= array();
			foreach($this->network->get_user_follows($this->user->id)->followers as $k=>$v) {
				$users[]	= intval($k);
			}
			if( $this->post_group ) {
			...
			$remove	= array();
			foreach($this->network->get_user_follows($this->post_user->id)->followers as $k=>$v) {
				$remove[]	= intval($k);
			}
		...
		public function show_share_link()
		{
			...
			if( $p->post_type == 'public' && (!$p->post_group || $p->post_group->is_public) ) {
				...
				if( isset($C->FACEBOOK_API_KEY) && !empty($C->FACEBOOK_API_KEY) ) {
					$html	.= '			
								<a class="shr_fb" onmouseover="dropcontrols_share_keepopen();" href="javascript:;" onclick="FB.Connect.showShareDialog(\''.htmlspecialchars($p->permalink).'\');">Facebook</a>';
				}
				else {
					$html	.= '			
								<a class="shr_fb" onmouseover="dropcontrols_share_keepopen();" href="http://www.facebook.com/sharer.php?u='.urlencode($p->permalink).'&t='.urlencode(htmlspecialchars($p->post_user->fullname.': '.$p->post_message)).'">Facebook</a>';
				}
				$html	.= '
								<a class="shr_tw" onmouseover="dropcontrols_share_keepopen();" href="'.$C->SITE_URL.'twitter-share/?url='.urlencode($p->permalink).'&status='.urlencode(htmlspecialchars($p->post_user->fullname.': '.$p->post_message)).'" target="_blank">Twitter</a>
								<a class="shr_bz" onmouseover="dropcontrols_share_keepopen();" href="http://www.google.com/reader/link?url='.urlencode($p->permalink).'&title='.urlencode(htmlspecialchars($p->post_user->fullname.': '.$p->post_message)).'&srcURL='.urlencode($p->permalink).'" target="_blank">Google Buzz</a>
								<a class="shr_dl" onmouseover="dropcontrols_share_keepopen();" href="javascript:;" onclick="window.open(\'http://delicious.com/save?v=5&noui&jump=close&url=\'+encodeURIComponent(\''.$p->permalink.'\')+\'&title=\'+encodeURIComponent(\''.str_replace("'","\'",htmlspecialchars($p->post_user->fullname.': '.$p->post_message)).'\'), \'delicious\',\'toolbar=no,width=550,height=550\'); return false;">Delicious</a>';
				return $html;
			}
			return '';
		}
